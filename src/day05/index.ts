...
class day05 extends Base<string[]> {
	private declare stack: stack;

	public run(part: number) {
		for (const step of this.input) {
			const [, amount, from, to] = step.match(/move (\d+) from (\d+) to (\d+)/)!;
			if (part === 1) for (let i = 0; i < +amount; i++) this.stack[+to].unshift(this.stack[+from].shift());
			else if (part === 2) this.stack[+to].unshift(...this.stack[+from].splice(0, +amount));
	...
	private parseStack(input: string): (string | undefined)[][] {
		const arr: stack = [];
